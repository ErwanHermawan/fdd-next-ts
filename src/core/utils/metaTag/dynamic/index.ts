...
const metaTagDynamic = (data: MetaDataInput = {}) => {
	const metadataBase = new URL(
		process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"
	);

	// Extract defaults from DefaultSEO
	const { title, description, keywords, siteURL, openGraph, twitter } =
		DefaultSEO;
	...
	return {
		...
		...(twitter.enable && {
			twitter: {
				...
				description: pageDescription,
				images: [
					new URL(data.twitterImage || twitter.image, metadataBase).toString(),
				],
			},
		}),
	};
